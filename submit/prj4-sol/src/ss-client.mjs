...
export default class SSClient {
  ...
  async updateCell(ssName, cellId, formula) {
    try {
      //@TODO
      const temp=await this.axios.patch(`${BASE}/${ssName}/${cellId}`,{formula:`${formula}`});
      return temp.data;
    }
    catch (err) {
      rethrow(err);
  ...
  async clear(ssName) {
    try {
      //@TODO
     const temp=await this.axios.delete(`${BASE}/${ssName}`);
      return temp.data;
    }
    catch (err) {
      rethrow(err);
  ...
  async delete(ssName, cellId) {
    try {
      //@TODO
      const temp=await this.axios.delete(`${BASE}/${ssName}/${cellId}`);
    //  return temp.data;
    }
    catch (err) {
      rethrow(err);
  ...
  async readFormulas(ssName) {
    try {
      //@TODO
      const temp=await this.axios.get(`${BASE}/${ssName}`);
      return temp.data;
    }
    catch (err) {
      rethrow(err);
